...
public class FlightsUseCase {
	...
	public List<InterconnectedFlights> findFlights(String departure, String arrival, LocalDateTime departureDateTime,
		...
		List<List<ryanair.flights.domain.model.Route>> routes = retrieveRoutes(departure, arrival);
		List<YearMonth> dates = getMonthsBetweenStartAndEnd(departureDateTime, arrivalDateTime);
		...
		for (List<ryanair.flights.domain.model.Route> routesLeg : routes) {
			flightsPerLeg = getFlightsLeg(departureDateTime, arrivalDateTime, dates, routesLeg);
			...
			if (flightsPerLeg.size() > 1) {
				...
				for (Leg legsFrom : legsListFrom) {
					for (Leg legsTo : legsListTo) {
						...
						if (legsFrom.getDepartureTime().isAfter(departureDateTime)
								&& legsTo.getDepartureTime().isAfter(legsFrom.getArrivalTime().plusMinutes(120))
								&& legsTo.getArrivalTime().isBefore(arrivalDateTime)
